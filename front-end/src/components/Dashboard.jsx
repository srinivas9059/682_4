...
function Dashboard() {
  ...
  // ▶️ Put this right below your useState hooks, before the summary-fetch effect
  useEffect(() => {
    async function loadFormDefinition() {
      try {
        const formID = localStorage.getItem("formID");
        const res = await fetch(`${BACKEND_URL}/getFormData/${formID}`);
        if (!res.ok) throw new Error("Couldn’t load form definition");
        const body = await res.json();
        // NOTE: the API returns { form: { formSections: […] , … } }
        const sections = body.form?.formSections;
        if (Array.isArray(sections) && sections.length) {
          setOriginalFormSections(sections);
        }
      } catch (err) {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (
            ...
            prepareOptions(data.formParentGroups, data.formGroups);
    ...
    // 🛠️ Filter logic to extract responses per group
    const getGroupResponses = (selectedGroups, groupData) => {
      const selected = Object.values(selectedGroups);
      return groupData.filter((g) => selected.includes(g.groupName));
    };

    const selectedGroupNames1 = selectedNodes1;
    const selectedGroupNames2 = selectedNodes2;

    const filteredResponses1 = getGroupResponses(
      selectedGroupNames1,
      groupsData
    );
    const filteredResponses2 = getGroupResponses(
      selectedGroupNames2,
      groupsData
    );
        ...
        Object.values(group.sections).forEach((section) => {
          ...
          section.questions.forEach((q) => {
            ...
            if (index === -1) {
              ...
              const newQ = {
                ...q,
                // start with all of the raw responses right away
                responses: [...q.responses],
                // leave subData alone (you re-compute it later in getCombinedResponses)
                subData:
                  q.questionType === 1
                    ? {}
                    : q.questionType === 3
              ...
              sectionMap[section.sectionID].questions.push(newQ);
            } else {
              // if it's already in the map, just append
  ...
  console.log("Sections in Dashboard:", originalFormSections);
  return (
