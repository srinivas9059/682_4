...
function MCQDashboardListItem({
  ...
  const dataPoints1 = content
    ? Object.entries(content.subData).map(([option, count]) => ({
        ...
        percentage:
          totalResponses1 > 0
            ? ((count / totalResponses1) * 100).toFixed(2)
            : 0,
      }))
    : [];
  ...
  const dataPoints2 = content2
    ? Object.entries(content2.subData).map(([option, count]) => ({
        ...
        percentage:
          totalResponses2 > 0
            ? ((count / totalResponses2) * 100).toFixed(2)
            : 0,
      }))
    : [];
  ...
  const series = content2
    ? [
        {
          data: dataPoints1.map((d) => d.percentage),
          label: `${firstGroupName} (%)`,
        },
        {
          data: dataPoints2.map((d) => d.percentage),
  ...
  const leastSelected =
    sortedData1.length > 1 ? sortedData1[sortedData1.length - 1] : null;

  let percentageDifference = null;
  if (sortedData1.length > 1) {
    const secondMostSelected = sortedData1[1];
    percentageDifference =
      ((mostSelected.count - secondMostSelected.count) / totalResponses1) * 100;
  ...
  let comparativeText = "";
  if (content2 && dataPoints1.length && dataPoints2.length) {
    const differences = dataPoints1.map((d, index) => {
      const second = dataPoints2[index] || { percentage: 0 };
      const preferredBy =
        d.percentage > second.percentage
          ? firstGroupName || "Group 1"
          : secondGroupName || "Group 2";
      return {
        option: d.option,
        diff: Math.abs(d.percentage - second.percentage),
        preferredBy,
      };
    });

    if (differences.length > 0) {
      const largestDiff = differences.reduce(
        (max, d) => (d.diff > max.diff ? d : max),
        differences[0]
      );
      ...
      differences.forEach((d) => {
        comparativeText += `<br/>${d.preferredBy} prefers <strong>${
          d.option
        }</strong> more (${d.diff.toFixed(2)}% gap).`;
      });
    }
  }
  ...
  const xAxisData = dataPoints1.map((d, index) => {
    if (content2) {
      return `${d.option}\n${firstGroupName}: ${
        d.count
      } | ${secondGroupName}: ${dataPoints2[index]?.count || 0}`;
    } else {
