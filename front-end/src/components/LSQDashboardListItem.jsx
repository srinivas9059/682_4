...
function LSQDashboardListItem({
    ...
    totalResponses > 0
      ? (
          content.responses.reduce(
            (sum, val) => sum + Math.pow(parseInt(val) - parseFloat(mean), 2),
            0
          ) / totalResponses
        ).toFixed(2)
      : 0;
  ...
  // How many responses came back for group #2?
  const totalResponses2 = content2?.responses?.length || 0;

  // If there are any responses, compute the mean; otherwise fall back to 0
  const mean2 =
    totalResponses2 > 0
      ? (
  ...
  // Only compute a percentage if you really have both a mean and an upperLimit
  const avgPercentage2 =
    totalResponses2 > 0 && content2.upperLimit
      ? ((parseFloat(mean2) / content2.upperLimit) * 100).toFixed(2)
      : "0.00";
